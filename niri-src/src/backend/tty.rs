...
impl Tty {
    ...
    fn device_added(
        ...
        let mut try_initialize_gpu = || -> anyhow::Result<DrmNode> {
            ...
            // In virtual machines without 3D HW acceleration (QEMU/KVM virtio, VirtualBox, etc.),
            // llvmpipe/softpipe is the only available renderer. Allow it so the compositor can run.
            if egl_device.is_software() {
                warn!("software EGL renderer detected (llvmpipe/softpipe); enabling virtual machine fallback");
            }
            ...
            if render_node != node {
                let _ = self.gpu_manager.as_mut().add_node(node, gbm.clone());
            }
