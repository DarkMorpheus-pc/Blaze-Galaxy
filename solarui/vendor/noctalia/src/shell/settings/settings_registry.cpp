...
namespace settings {
  namespace {
    ...
    constexpr std::array<SettingsSectionDescriptor, 23> kSettingsSections{{
        ...
        {SettingsSection::Niri, "niri", "niri", false},
        {SettingsSection::SolarUI, "solarui", "sun"},
  ...
  std::vector<SettingEntry> buildSettingsRegistry(
    ...
    // Compositor & Backdrop integrations under SolarUI
    if (env.niriOverviewTypeToLaunchSupported || env.niriBackdropSupported) {
      if (env.niriOverviewTypeToLaunchSupported) {
        entries.push_back(makeEntry(
            SettingsSection::SolarUI, "compositor", tr("settings.schema.shell.niri-overview-type-to-launch.label"),
            ...
            "solarui niri overview type launch launcher search keyboard focus"
        ));
      }
      if (env.niriBackdropSupported) {
        entries.push_back(makeEntry(
            SettingsSection::SolarUI, "compositor", tr("settings.schema.shared.enabled.label"),
            tr("settings.schema.backdrop.enabled.description"), {"backdrop", "enabled"},
            ToggleSetting{cfg.backdrop.enabled}, "wallpaper backdrop"
        ));
        entries.push_back(makeEntry(
            SettingsSection::SolarUI, "compositor", tr("settings.schema.backdrop.blur-intensity.label"),
            tr("settings.schema.backdrop.blur-intensity.description"), {"backdrop", "blur_intensity"},
            sliderFor(cfg.backdrop.blurIntensity, noctalia::config::schema::kUnitRange, false), "wallpaper blur"
        ));
        entries.push_back(makeEntry(
            SettingsSection::SolarUI, "compositor", tr("settings.schema.backdrop.tint-intensity.label"),
            tr("settings.schema.backdrop.tint-intensity.description"), {"backdrop", "tint_intensity"},
            sliderFor(cfg.backdrop.tintIntensity, noctalia::config::schema::kUnitRange, false), "wallpaper tint"
        ));
      }
    }

    // SolarUI Desktop Platform Settings
    // Shell Engine & Surface Providers
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "shell", tr("settings.schema.solarui.shell-engine.label"),
        tr("settings.schema.solarui.shell-engine.description"),
        {"solarui", "shell_engine"},
        asSegmented(plainSelect(
            {{"noctalia", "settings.options.solarui.shell-engine.noctalia"},
             {"caelestia", "settings.options.solarui.shell-engine.caelestia"},
             {"hybrid", "settings.options.solarui.shell-engine.hybrid"}},
            cfg.solarui.shellEngine
        )),
        "solarui shell engine quickshell noctalia caelestia hybrid"
    ));
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "shell", tr("settings.schema.solarui.apply-shell-engine.label"),
        tr("settings.schema.solarui.apply-shell-engine.description"),
        {"solarui", "apply_shell"},
        ButtonSetting{
            .label = tr("settings.schema.solarui.apply-shell-engine.button"),
            .action = [engine = cfg.solarui.shellEngine]() {
                std::string cmd = "solar-shell switch " + engine + " &";
                std::system(cmd.c_str());
            },
            .glyph = "restart",
            .variant = ButtonVariant::Primary,
        },
        "solarui apply switch shell engine"
    ));
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "shell", tr("settings.schema.solarui.launcher-provider.label"),
        tr("settings.schema.solarui.launcher-provider.description"),
        {"solarui", "launcher_provider"},
        asSegmented(plainSelect(
            {{"noctalia", "settings.options.solarui.launcher-provider.noctalia"},
             {"caelestia", "settings.options.solarui.launcher-provider.caelestia"}},
            cfg.solarui.launcherProvider
        )),
        "solarui launcher provider surface"
    ));
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "shell", tr("settings.schema.solarui.dashboard-provider.label"),
        tr("settings.schema.solarui.dashboard-provider.description"),
        {"solarui", "dashboard_provider"},
        asSegmented(plainSelect(
            {{"caelestia", "settings.options.solarui.dashboard-provider.caelestia"},
             {"noctalia", "settings.options.solarui.dashboard-provider.noctalia"}},
            cfg.solarui.dashboardProvider
        )),
        "solarui dashboard provider surface"
    ));

    // Performance Profile
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "performance", tr("settings.schema.solarui.performance-profile.label"),
        tr("settings.schema.solarui.performance-profile.description"),
        {"solarui", "performance_profile"},
        asSegmented(plainSelect(
            {{"legacy", "settings.options.solarui.performance-profile.legacy"},
             {"balanced", "settings.options.solarui.performance-profile.balanced"},
             {"ultra", "settings.options.solarui.performance-profile.ultra"}},
            cfg.solarui.performanceProfile
        )),
        "solarui performance profile legacy balanced ultra"
    ));

    // Pixel Clock
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "pixel-clock", tr("settings.schema.solarui.clock-enabled.label"),
        tr("settings.schema.solarui.clock-enabled.description"),
        {"solarui", "clock_enabled"}, ToggleSetting{cfg.solarui.clockEnabled},
        "solarui pixel clock desktop time widget"
    ));
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "pixel-clock", tr("settings.schema.solarui.clock-style.label"),
        tr("settings.schema.solarui.clock-style.description"),
        {"solarui", "clock_style"},
        asSegmented(plainSelect(
            {{"stacked_vertical", "settings.options.solarui.clock-style.stacked-vertical"},
             {"bold_horizontal", "settings.options.solarui.clock-style.bold-horizontal"},
             {"outline", "settings.options.solarui.clock-style.outline"},
             {"analog_bold", "settings.options.solarui.clock-style.analog-bold"},
             {"analog_outline", "settings.options.solarui.clock-style.analog-outline"},
             {"analog_tachymeter", "settings.options.solarui.clock-style.analog-tachymeter"}},
            cfg.solarui.clockStyle
        )),
        "solarui clock style stacked vertical bold horizontal outline analog tachymeter"
    ));
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "pixel-clock", tr("settings.schema.solarui.clock-opacity.label"),
        tr("settings.schema.solarui.clock-opacity.description"),
        {"solarui", "clock_opacity"},
        sliderFor(cfg.solarui.clockOpacity, noctalia::config::schema::kUnitRange, false),
        "solarui clock opacity alpha transparency"
    ));
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "pixel-clock", tr("settings.schema.solarui.clock-battery.label"),
        tr("settings.schema.solarui.clock-battery.description"),
        {"solarui", "clock_battery"}, ToggleSetting{cfg.solarui.clockBattery},
        "solarui clock battery pill indicator"
    ));
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "pixel-clock", tr("settings.schema.solarui.clock-date.label"),
        tr("settings.schema.solarui.clock-date.description"),
        {"solarui", "clock_date"}, ToggleSetting{cfg.solarui.clockDate},
        "solarui clock calendar date day"
    ));
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "pixel-clock", tr("settings.schema.solarui.clock-color-mode.label"),
        tr("settings.schema.solarui.clock-color-mode.description"),
        {"solarui", "clock_color_mode"},
        asSegmented(plainSelect(
            {{"dynamic", "settings.options.solarui.clock-color-mode.dynamic"},
             {"white", "settings.options.solarui.clock-color-mode.white"},
             {"monochrome", "settings.options.solarui.clock-color-mode.monochrome"}},
            cfg.solarui.clockColorMode
        )),
        "solarui clock color mode dynamic material wallpaper white"
    ));

    // Taskbar Settings
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "taskbar", tr("settings.schema.solarui.taskbar-preset.label"),
        tr("settings.schema.solarui.taskbar-preset.description"),
        {"solarui", "taskbar_preset"},
        asSegmented(plainSelect(
            {{"frosted_glass", "settings.options.solarui.taskbar-preset.frosted-glass"},
             {"solid_obsidian", "settings.options.solarui.taskbar-preset.solid-obsidian"},
             {"transparent", "settings.options.solarui.taskbar-preset.transparent"},
             {"m3_dynamic", "settings.options.solarui.taskbar-preset.m3-dynamic"}},
            cfg.solarui.taskbarPreset
        )),
        "solarui taskbar preset style frosted glass obsidian transparent material dynamic"
    ));
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "taskbar", tr("settings.schema.solarui.taskbar-opacity.label"),
        tr("settings.schema.solarui.taskbar-opacity.description"),
        {"solarui", "taskbar_opacity"},
        sliderFor(cfg.solarui.taskbarOpacity, noctalia::config::schema::kUnitRange, false),
        "solarui taskbar opacity alpha transparency"
    ));
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "taskbar", tr("settings.schema.solarui.taskbar-radius.label"),
        tr("settings.schema.solarui.taskbar-radius.description"),
        {"solarui", "taskbar_radius"},
        barCornerSlider(static_cast<std::int32_t>(cfg.solarui.taskbarRadius)),
        "solarui taskbar corner radius roundness"
    ));
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "taskbar", tr("settings.schema.solarui.taskbar-thickness.label"),
        tr("settings.schema.solarui.taskbar-thickness.description"),
        {"solarui", "taskbar_thickness"},
        barReservedSlider(static_cast<double>(cfg.solarui.taskbarThickness), 120.0, 1.0, true),
        "solarui taskbar thickness height size"
    ));

    // Window Management Settings
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "window-management", tr("settings.schema.solarui.window-mode.label"),
        tr("settings.schema.solarui.window-mode.description"),
        {"solarui", "window_mode"},
        asSegmented(plainSelect(
            {{"kde_floating", "settings.options.solarui.window-mode.kde-floating"},
             {"niri_tiling", "settings.options.solarui.window-mode.niri-tiling"}},
            cfg.solarui.windowMode
        )),
        "solarui window mode kde floating niri tiling scrolling"
    ));

    // Shortcuts & Quick HUD
    entries.push_back(makeEntry(
        SettingsSection::SolarUI, "help-hud", tr("settings.schema.solarui.shortcuts-hud.label"),
        tr("settings.schema.solarui.shortcuts-hud.description"),
        {},
        ButtonSetting{
            .label = tr("settings.schema.solarui.shortcuts-hud.action"),
            .action = [] {
              (void)process::runAsync({"sh", "-c", "solar-shell hud || true"});
            },
        },
        "solarui shortcuts cheat sheet hud guide help"
    ));
