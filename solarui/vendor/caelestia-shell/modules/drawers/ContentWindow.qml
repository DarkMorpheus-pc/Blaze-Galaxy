...
StyledWindow {
    ...
    readonly property int dragMaskPadding: {
        ...
        // In SolarUI / Niri, Hyprland monitor IPC is null.
        // Return 0 so Caelestia does not create a 30px invisible perimeter hit-mask that blocks window corner clicks and edge dragging.
        if (!monitor || monitor?.lastIpcObject?.specialWorkspace?.name || (monitor?.activeWorkspace?.lastIpcObject?.windows ?? 1) > 0)
            return 0;
